...
if audio_file:
    with st.spinner("⚡ Scansione tecnica in corso..."):
        ...
        # Estrazione Dati (Fix BPM)
        tempo_result = librosa.beat.beat_track(y=y, sr=sr)
        bpm_val = tempo_result if isinstance(tempo_result, (float, int)) else tempo_result
        ...
        if prompt := st.chat_input("Chiedi i settaggi di Ableton o scrivi: 'Generami un testo emozionale'"):
            if user_key:
                ...
                MISSIONE:
                1. CRITICA SPIETATA: Se i dati sono scarichi (LUFS > -10), insulta costruttivamente il mix.
                2. ABLETON NATIVE: Suggerisci settaggi precisi solo per plugin nativi (Glue Comp, Saturator, OTT, Reverb).
                3. SONGWRITING: Se richiesto, scrivi testi in inglese emozionale e suggerisci la melodia basata sulla scala {key_detected}.
                4. SETTAGGI: Fornisci tabelle (es. Threshold -20dB, Attack 10ms).
                5. CHIEDI: 'Come hai impostato il tuo [Plugin]?' per forzare l'utente a migliorare.
                """
...
with st.expander("📖 Glossario per Produttori Pro"):
    st.markdown("""
    *   **LUFS:** Il volume percepito. Garrix punta a -6/-8 LUFS. Se sei a -17, il tuo pezzo sparisce.
    *   **Crest Factor:** La differenza tra picco e media. Sotto 8dB il mix è un muro (EDM), sopra 12dB è troppo moscio.
    *   **OTT:** Compressione multibanda estrema. Il segreto del suono 'frizzante' dei Lead Progressive.
    *   **Glue Compressor:** Basato su console SSL. Si usa sul Master o sui Bus per 'incollare' gli strumenti.
    """)

if not audio_file:
    st.info("👋 Carica uno stem o il mix per iniziare. L'Ingegnere ti sta aspettando (ed è di cattivo umore).")
