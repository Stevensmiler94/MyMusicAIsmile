...
# --- CONFIGURAZIONE ---
st.set_page_config(page_title="AI Studio Pro: Platinum Elite", layout="wide", page_icon="🎹")
...
def get_platinum_stats(file_bytes):
    ...
    sub_mono_corr = float(np.corrcoef(y_low_L, y_low_R)[0, 1])
    
    # Stereo & Stats
    corr_val = float(np.corrcoef(y_stereo[0], y_stereo[1])[0, 1])
    ...
    return {"y": y_mono, "y_s": y_stereo, "sr": sr, "lufs": float(lufs), "crest": float(crest), "air": float(air_val), 
            "psd": np.mean(S, axis=1), "freqs": freqs, "width": width_val, "sub_mono": sub_mono_corr, "bands": bands}
...
with t2:
    ...
    if f_mix:
        ...
        c1.metric("Loudness", f"{d['lufs']:.1f} LUFS"); c2.metric("Air", f"{d['air']*100:.1f}%"); c3.metric("Width", f"{d['width']:.1f}%")
        c4.metric("Sub Mono", f"{d['sub_mono']:.2f}"); c5.metric("Focus", max(d['bands'], key=d['bands'].get))

        v1, v2, v3 = st.columns(3)
        with v1: 
            fig1, ax1 = plt.subplots(); librosa.display.waveshow(d['y'], sr=d['sr'], ax=ax1); st.pyplot(fig1)
        with v2:
            fig2, ax2 = plt.subplots(); ax2.scatter(d['y_s'][0, ::120], d['y_s'][1, ::120], s=1, color="lime", alpha=0.3)
            ax2.set_xlim(-0.8, 0.8); ax2.set_ylim(-0.8, 0.8); st.pyplot(fig2)
        with v3:
...
with t3:
    ...
    if f1 and f2:
        ...
        ax_eq.semilogx(d1['freqs'], librosa.amplitude_to_db(d1['psd']), label="Mio", color="cyan")
        ax_eq.semilogx(d2['freqs'], librosa.amplitude_to_db(d2['psd']), label="Ref", color="orange", alpha=0.6)
        ax_eq.set_xlim(20, 20000); ax_eq.legend(); st.pyplot(fig_eq)
        ...
        if st.button("🚀 Strategia Mastering"):
            if api_key:
                client = OpenAI(api_key=api_key)
                ctx = f"MIO: {d1['lufs']:.1f}LUFS. REF: {d2['lufs']:.1f}LUFS."
