import streamlit as st
import librosa
import librosa.display
import numpy as np
import matplotlib.pyplot as plt
import pyloudnorm as pdn
import openai
from audio_recorder_streamlit import audio_recorder
import json

# Configurazione Pagina
st.set_page_config(page_title="AI Studio Vault", layout="wide", page_icon="💾")

# --- INIZIALIZZAZIONE SESSIONE ---
if "progetti" not in st.session_state:
    st.session_state.progetti = {"Default": []}
if "progetto_attivo" not in st.session_state:
    st.session_state.progetto_attivo = "Default"

# --- FUNZIONI DI SALVATAGGIO/CARICAMENTO ---
def export_progetto():
    # Prepara i dati del progetto corrente per il download
    data = {
        "nome": st.session_state.progetto_attivo,
        "chat": st.session_state.progetti[st.session_state.progetto_attivo]
    }
    return json.dumps(data, indent=4)

def import_progetto(uploaded_json):
    if uploaded_json:
        data = json.load(uploaded_json)
        nome = data["nome"]
        st.session_state.progetti[nome] = data["chat"]
        st.session_state.progetto_attivo = nome
        st.toast(f"Progetto '{nome}' caricato con successo!")

# --- SIDEBAR: STUDIO MANAGER ---
st.sidebar.title("🏢 Studio Vault")
user_key = st.sidebar.text_input("OpenAI API Key", type="password")

# Gestione Progetti Locali
st.sidebar.subheader("📁 Gestione Scenari")
st.session_state.progetto_attivo = st.sidebar.selectbox("Seleziona Progetto Attivo", list(st.session_state.progetti.keys()))

if st.sidebar.button("🗑️ Reset Chat Corrente"):
    st.session_state.progetti[st.session_state.progetto_attivo] = []
    st.rerun()

st.sidebar.divider()

# --- IMPORT / EXPORT (SALVA SU PC) ---
st.sidebar.subheader("💾 Backup su PC")
# Tasto Download
json_string = export_progetto()
st.sidebar.download_button(
    label="📥 Salva Progetto (.json)",
    data=json_string,
    file_name=f"{st.session_state.progetto_attivo}_backup.json",
    mime="application/json"
)

# Tasto Upload
file_caricato = st.sidebar.file_uploader("📤 Carica Progetto (.json)", type=["json"])
if file_caricato:
    import_progetto(file_caricato)

st.sidebar.divider()
nuovo_p = st.sidebar.text_input("Nome nuovo scenario")
if st.sidebar.button("Crea Nuovo Scenario"):
    if nuovo_p and nuovo_p not in st.session_state.progetti:
        st.session_state.progetti[nuovo_p] = []
        st.session_state.progetto_attivo = nuovo_p
        st.rerun()

# --- INTERFACCIA PRINCIPALE ---
st.title(f"🎙️ Scenario: {st.session_state.progetto_attivo}")

# Registratore Vocale
audio_bytes = audio_recorder(text="Registra idea melodica", icon_size="2x")
if audio_bytes:
    st.audio(audio_bytes, format="audio/wav")

# Analisi Tecnica Audio
st.divider()
uploaded_file = st.sidebar.file_uploader("Analizza Mix/Stem (Opzionale)", type=["wav", "mp3"])

tech_context = ""
if uploaded_file:
    with st.spinner("⚡ Analisi tecnica..."):
        y, sr = librosa.load(uploaded_file, duration=30)
        tempo = float(np.atleast_1d(librosa.beat.beat_track(y=y, sr=sr)))
        lufs = pdn.Meter(sr).integrated_loudness(y.reshape(-1, 1) if y.ndim == 1 else y.T)
        crest = 20 * np.log10(np.max(np.abs(y)) / (np.sqrt(np.mean(y**2)) + 1e-9))
        st.info(f"📊 Dati Rilevati: {lufs:.1f} LUFS | {crest:.1f}dB Crest | {int(tempo)} BPM")
        tech_context = f"[ANALISI TECNICA: {lufs:.1f} LUFS, {crest:.1f}dB Crest, {int(tempo)} BPM]. "

# --- CHAT INTERATTIVA ---
st.divider()
for msg in st.session_state.progetti[st.session_state.progetto_attivo]:
    with st.chat_message(msg["role"]): st.markdown(msg["content"])

if prompt := st.chat_input("Scrivi qui le tue domande o idee..."):
    if user_key:
        openai.api_key = user_key
        
        sys_instr = f"""
        Sei un Senior Producer Progressive House (Ableton Expert). 
        USA QUESTI DATI: {tech_context}. 
        Se l'utente dà accordi, disegna il PIANO ROLL con griglie | X | - | . 
        Sii critico su LUFS e Crest. Suggerisci plugin Ableton e parametri precisi.
        Se chiedono testi, scrivi in inglese emozionale.
        """
        
        st.session_state.progetti[st.session_state.progetto_attivo].append({"role": "user", "content": prompt})
        with st.chat_message("user"): st.markdown(prompt)

        with st.chat_message("assistant"):
            try:
                resp = openai.ChatCompletion.create(
                    model="gpt-4o-mini",
                    messages=[{"role": "system", "content": sys_instr}] + st.session_state.progetti[st.session_state.progetto_attivo]
                )
                answer = resp.choices.message.content
                st.markdown(answer)
                st.session_state.messages.append({"role": "assistant", "content": answer}) # Per compatibilità
                st.session_state.progetti[st.session_state.progetto_attivo].append({"role": "assistant", "content": answer})
            except Exception as e:
                st.error(f"Errore: {e}")
    else:
        st.warning("⚠️ Inserisci la API Key!")
